        ...
        If (CondRefOf(\RMCF.DPTS))
        {
            If (\RMCF.DPTS)
            {
                ...
                If (CondRefOf(\_SB.PCI0.RP01.PXSX._ON)) {
                  \RMDT.PUSH("sakoula _pts gpu on")
                  \_SB.PCI0.RP01.PXSX._ON()
                } else {
                  \RMDT.PUSH("sakoula _pts gpu on method not found")
                }
            }
        }
        ...
        If (CondRefOf(\RMCF.DPTS))
        {
            If (\RMCF.DPTS)
            {
                ...
                If (CondRefOf(\_SB.PCI0.RP01.PXSX._OFF)) {
                  \RMDT.PUSH("sakoula _wak gpu off")
                  \_SB.PCI0.RP01.PXSX._OFF()
                } else {
                  \RMDT.PUSH("sakoula _wak gpu off method not found")
                }
            }
        }
